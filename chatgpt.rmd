...
En esta sección, profundizaremos en la aplicación del algoritmo Random Forest, 
implementado a través del paquete ranger en R, para abordar el problema de la 
aprobación de créditos utilizando el conjunto de datos "Credit Approval" de la UCI. 
La elección de Random Forest para esta tarea no es arbitraria; se basa en sus robustas características 
y su probada eficacia en problemas de clasificación complejos, como el que nos ocupa.
```{r}
set.seed(68)
...
pred_ranger <- predict(credit.ranger, newdata = credit.Datos.Test)
