...
ggplot(data.frame(Faltantes = con_na), aes(x = Faltantes)) +
  geom_bar(fill = "steelblue") +transformacion
...
Tras probar las 4 alternativas, nos dimos cuenta que no existía una gran diferencia entre ellas, obteniendo la opción
sin transformaciones unos resultados prácticamente idénticos al resto para todos los modelos. Es por ello que decidimos 
proseguir sin introducir transformaciones generales en los datos.
...
Las seis componentes aparecen ordenadas según la proporción de la varianza explicada (PVE).
La PVE de un componente coincide con la proporción del autovalor m-ésimo
...
credit_no_na_esfuerzo[[credit.Var.Salida.Usada]] <- as.factor(credit_no_na_esfuerzo[[credit.Var.Salida.Usada]])
...
Como primera aproximación a la tarea de clasificación, nos pareció interesante usar una técnica sencilla como 
lo son los árboles de decisión. El objetivo de este ataque, más que obtener una solución final, es establecer 
un clasificador base contra el que contrastar resultados porsteriores que obtendremos con otros modelos.
```{r}
library(rpart)
...
Hemos hecho uso de `rpart`, con un `tuneLength` de 10. Hemos probado distintos valores 
...
Vamos a hacer una exploración para el número de árboles, para la profundidad máxima de los mismos,
la tasa de aprendizaje y mínimo de observaciones por nodo.
Finalmete, con mtry indicamos el número de variables utilizadas aleatoriamente como predictores.
...
El resultado óptimo de accuracy (0.8790174) se ha obtenido con los siguiente parámetros:
...
El resultado más sorprendente es que el modelo está tomando un interaction depth de 1, 
resultando extremadamente simple. Es decir, está utilizando únicamente una variable como
predictor. Podemos ver la evolución de la acurracy en función de la tasa de aprendizaje
para todas las combinaciones de parámetros:
...
Nuevamente, el impago previo es la variable más determinante, de ahí que se esté tomando una
profundidad de 1. Los predictores se están haciendo con este campo.
