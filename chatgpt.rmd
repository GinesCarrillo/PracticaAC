...
# Función para mostrar R^2 en los paneles superiores
  cor_plus_r2_fun <- function(data, mapping, ...) {
    ...
    r_plus  <- tryCatch(cor(df$x[df$clase == "+"], df$y[df$clase == "+"]), error = function(e) NA)
    r_minus <- tryCatch(cor(df$x[df$clase == "-"], df$y[df$clase == "-"]), error = function(e) NA)
...
Estas correlaciones son más intensas en la clase `+`. Esto suguiere que los
...
```{r}
subset_credit$Esfuerzo_financiero <- subset_credit$Deuda + subset_credit$Anos_cotizados

x11()
ggplot(subset_credit, aes(x = Esfuerzo_financiero, fill = Aprobado)) +
  geom_density(alpha = 0.5) +
  theme_minimal() +
  labs(title = "Distribución del índice combinado log10(Deuda+1) + log10(Años cotizados+1)",
       x = "Índice esfuerzo financiero", y = "Densidad")
      
```

Hemos llamado a la suma de ambos atributos (en escala logarítmica) `Esfuerzo_financiero`.
...
También destaca la diferencia entre una deuda alta y baja. De hecho, si normalizamos la escala de la los ingresos para que abarquen
sumamos la diferencia de los ingresos con respecto a la moda 10 con el campo `Anos_cotizados` (escalando el primer valor para compartir rango),
el resultado es la siguiente distribución:

```{r}
subset_credit$Esfuerzo_financiero <- abs(subset_credit$Ingresos-1)*0.375 + subset_credit$Anos_cotizados
